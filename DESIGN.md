...
Photo paths are `/SpreadPaper/photos/NAME.jpg`, with the prefix. Verified against the dev server and a production build: that form returns the image and survives the build unchanged, while `/photos/NAME.jpg` is a 404. The unprefixed form looks like it works if you only check the status code, because the dev server answers an unknown path with the HTML fallback at 200.
...
- types, Dynamic row: four panels `16` / `16` / `16` / `16`, narrow, with the four `hero-day-*.jpg` images stepping through, and a time readout in `text-cd-dynamic` beside it.
...
Every interactive element gets `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cd-accent focus-visible:ring-offset-2 focus-visible:ring-offset-cd-bg`. Tap targets 44px. The page must work at 375px with a 20px gutter, which `.cd-section` already gives; nothing may scroll the body sideways. One `<h1>`, landmarks for `nav`, `main`, `footer`, and section headings in document order.
