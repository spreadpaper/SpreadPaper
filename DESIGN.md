...
- gallery: small spreads inside `.cd-card`, three panels at `aspect-[16/3]`, with a type badge in the corner. The cards run 389px and 492px wide, so that lands at 73px and 92px tall, which is a thumbnail rather than a ribbon. My earlier `aspect-[16/6]` here drew three portrait slots, and the two-panel version I then proposed was over-correction on an assumption about card width I had not checked.
