...
- **Views/SpreadPhoto.swift** — `PanelSlice`, `SpreadContent` and `SpreadPhoto`: one scene laid out across several illustration panels, each panel clipped to its own part of it. Used by the creation modal hero and the wizard. Also `HeroPhoto`, the two bundled photographs with their attribution, and `HeroCrossfade` (pure, unit-tested), the day-to-night loop the Light & Dark hero reads off the wall clock
